...
# 테스트할 원본 이미지
image_path = "/home/minsub/Downloads/tera.jpg"
# 좌표 데이터 설정 (예제 좌표)
coordinates_positive = [{"x": 256, "y": 256}, {"x": 241, "y": 149}, {"x": 250, "y": 385}, {"x": 274, "y": 419}]
coordinates_negative = [{"x": 82, "y": 86}]
# 테스트 이미지 base64 인코딩
...
if not check_segmentation_status():
    ...
    response = requests.post(url + "api/segmentation/point_start", data=json.dumps(data), headers=headers)

    with open('./seg_point_input_json.json', 'w') as f:
        json.dump(data, f)
    ...
    if response.status_code == 200:
        ...
        with open('./seg_point_output_json.json', 'w') as f:
        ...
        if response_data.get('status') == 'success':
            ...
            result_image.save('./point_result.png')
